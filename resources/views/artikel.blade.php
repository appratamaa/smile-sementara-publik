...
<body class="h-full bg-white">
    <div class="min-h-full ">
        ...
        <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            ...
            <p class="mt-4 text-base text-gray-600">Temukan berbagai artikel tentang kesehatan gigi dan mulut untuk
                meningkatkan pengetahuan dan menjaga kebersihan gigi Anda.</p>
        </div>
            ...
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <form action="{{ url('/artikel/cari') }}" method="GET" class="mt-6">
                    <div class="relative">
                        ...
                        <button type="submit"
                            class="absolute right-2 top-1/2 transform -translate-y-1/2 bg-blue-500 text-white px-3 py-1 rounded-md hover:bg-blue-600">Cari</button>
                    </div>
                </form>

                <!-- Filter & Switch Tampilan -->
                <div class="mt-6" x-data="{ gridView: true }">
                    <div class="flex flex-wrap justify-between gap-4 mb-6">
                        <div>
                            <button @click="gridView = true" :class="{ 'bg-blue-500 text-white': gridView }"
                                class="px-4 py-2 border rounded-md hover:bg-blue-500 hover:text-white">
                                <i class="fa-solid fa-th"></i> Grid
                            </button>
                            <button @click="gridView = false" :class="{ 'bg-blue-500 text-white': !gridView }"
                                class="px-4 py-2 border rounded-md hover:bg-blue-500 hover:text-white">
                        ...
                        @foreach ($artikels as $artikel)
                            <a href="{{ url('/artikel/' . $artikel['id_artikel']) }}"
                                ...
                                <img src="{{ asset('gambar_artikel/' . $artikel['gambar']) }}"
                                    alt="{{ $artikel['judul_artikel'] }}" class="w-full h-40 object-cover">
                                <div class="p-4">
                                    <h3 class="text-lg font-bold">{{ $artikel['judul_artikel'] }}</h3>
                                    <p class="mt-2 text-gray-600">{{ Str::limit($artikel['deskripsi_artikel'], 120) }}
                                    </p>
                                </div>
                            </a>
                        @endforeach
                    ...
                    <!-- Row View -->
                    <div x-show="!gridView" class="space-y-6">
                        @foreach ($artikels as $artikel)
                            <a href="{{ url('/artikel/' . $artikel['id_artikel']) }}"
                                class="flex flex-col md:flex-row bg-white shadow-md rounded-lg overflow-hidden hover:shadow-lg transition">
                                <img src="{{ asset('gambar_artikel/' . $artikel['gambar']) }}"
                                    alt="{{ $artikel['judul_artikel'] }}" class="w-full md:w-1/3 h-28 object-cover">
                                <div class="p-4 flex-1">
                                    <h3 class="text-lg font-bold">{{ $artikel['judul_artikel'] }}</h3>
                                    <p class="mt-2 text-gray-600">{{ Str::limit($artikel['deskripsi_artikel'], 200) }}
                                    </p>
                                </div>
                            </a>
                        @endforeach
